      ...
      <a-list-item slot='renderItem' key='item.title' slot-scope='item'>
        <template slot='actions'>
          <span style='display: flex;align-items: center;gap: 5px'>
                <a-icon type='like-o' @click='likeItemInfo(item)' v-if='!item.like'></a-icon>
                <a-icon type='like-o' @click='unlikeItemInfo(item)' v-if='item.like' style='color: red'></a-icon>
...
<script>
import { likeItem, myLikeItems, unlikeItem } from '@/api/item'
...
export default {
  ...
  methods: {
    ...
    async getPublishItems (loading = true) {
      ...
      const { data } = await myLikeItems(this.itemVo)
